...
@section('content')
    @php
        $delete_url_path = '/admin/company/site/delete/';
    @endphp
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/company/dashboard')}}">{{ trans('labels.dashboard') }}</a></li>
        <li class="breadcrumb-item active">{{ trans('labels.site_list') }}</li>
        <!-- Breadcrumb Menu-->
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('labels.site_list') }}
                    </div>
                    <div class="card-body">
                        <table class="table table-responsive-sm table-bordered table-striped table-sm">
                            ...
                            @foreach($record as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $edit_url_path = '/admin/company/site_form?id='.GeneralFunctions::encryptString($value['Id']);
                                        @endphp
                                        <div class="btn-group" role="group">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                @if(GeneralFunctions::check_edit_permission('site_list'))
                                                    <a href="{{url($edit_url_path)}}" class="dropdown-item"><i class="fa fa-edit"></i> {{ trans('labels.edit') }}</a>
                                                @endif
                                                @if(GeneralFunctions::check_delete_permission('site_list'))
    ...
    <div class="remodal" data-remodal-id="delete_modal"
        ...
        <h1>{{ trans('labels.remove_site') }}</h1>
        <p>
