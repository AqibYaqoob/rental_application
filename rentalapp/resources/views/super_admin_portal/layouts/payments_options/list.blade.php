...
@section('content')
    @php
        $delete_url_path = '/admin/payments/delete/';
    @endphp
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/dashboard')}}">{{ trans('labels.dashboard') }}</a></li>
        <li class="breadcrumb-item active">{{ trans('labels.payments_list') }}</li>
        <!-- Breadcrumb Menu-->
    </ol>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('labels.payments_list') }}
                    </div>
                    <div class="card-body">
                        <table class="table table-responsive-sm table-bordered table-striped table-sm">
                            <thead>
                            <tr>
                                <th>{{ trans('labels.payment_name') }}</th>
                            ...
                            @foreach($record as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $edit_url_path = '/admin/payments?id='.GeneralFunctions::encryptString($value['id']);
                                        @endphp
                                        <div class="btn-group" role="group">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                @if(GeneralFunctions::check_edit_permission('payments_list'))
                                                    <a href="{{url($edit_url_path)}}" class="dropdown-item"><i class="fa fa-edit"></i> {{ trans('labels.edit') }}</a>
                                                @endif
                                                @if(GeneralFunctions::check_delete_permission('payments_list'))
    ...
    <div class="remodal" data-remodal-id="delete_modal"
        ...
        <h1>{{ trans('labels.remove_payments') }}</h1>
        <p>
