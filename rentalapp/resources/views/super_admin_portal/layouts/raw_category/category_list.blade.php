...
@section('content')
    @php
        $delete_url_path = '/admin/raw_category/delete/';
    @endphp
    <ol class="breadcrumb">
                        ...
                        <table class="table table-responsive-sm table-bordered table-striped table-sm">
                            ...
                            @foreach($record as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $edit_url_path = '/admin/item_category_form?id='.GeneralFunctions::encryptString($value['id']);
                                        @endphp
                                        <div class="btn-group" role="group">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                @if(GeneralFunctions::check_edit_permission('item_category_list'))
                                                    <a href="{{url($edit_url_path)}}" class="dropdown-item"><i class="fa fa-edit"></i> {{ trans('labels.edit') }}</a>
                                                @endif
                                                @if(GeneralFunctions::check_delete_permission('item_category_list'))
